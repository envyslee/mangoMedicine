...
public class Images {
    ...
    public static List<CategoryItem> Get_Product(int index) {
        List<CategoryItem> categoryItems = new ArrayList<>();
        int length=index*10+10;
        if (length>imageUrls.length){
            length=imageUrls.length;
        }
        for (int i = index*10; i < length; i++) {
            Random random=new Random();
            DecimalFormat df=new DecimalFormat("0.00");
            String price=df.format(random.nextDouble());
            int sales=random.nextInt(10000);
            CategoryItem item = new CategoryItem("药品" + (i+1), "这是药品的简要介绍这是药品的简要介绍这是药品的简要介绍", imageUrls[i],price,sales,"10ml/支x10支/盒");
            categoryItems.add(item);
        }
        return categoryItems;
    }

    public final static List<CategoryItem> Get_LIST_DATAS() {
        List<CategoryItem> categoryItems = new ArrayList<>();
        for (int i = 0; i < productImgs.length; i++) {
            CategoryItem item = new CategoryItem("Num" + i, "这是二级标题这是二级标题这是二级标题这是二级标题这是二级标题这是二级标题", productImgs[i]);
            categoryItems.add(item);
        }
        return categoryItems;
    }

    public final static String[] productImgs=new String[]{
